...
-- database rebuilt from it had the outbox with NO PRODUCER: the tables
-- existed and nothing ever wrote to them, silently and with no error. The
-- triggers on public.quotes are what generate every event, so they belong
-- here alongside the tables. Copied from
-- migrations/2026-08-22-quote-lifecycle-event-outbox.sql, which stays the
