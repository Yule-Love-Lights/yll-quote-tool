...
--
-- The customer pays their 50% deposit on Valor's hosted payment page (we redirect
-- them there from the portal). Valor confirms the payment with a signed webhook;
-- on that confirmation we stamp these columns, move the HighLevel card to
-- ⏰Approved, and email the receipt. Staff later charge the remaining 50% balance
-- MANUALLY in the Valor portal using the vaulted card — we build no auto-charge.
--
...
--
-- NOTE: already APPLIED to the live Supabase (S13). This file is the repo record
-- of that change so a fresh DB reproduces it.

BEGIN;

-- Our reference, sent to Valor's hosted-page sale as the invoice number and
-- echoed back by the webhook, so the webhook can map the payment to this quote
-- (same "round-trip our id" pattern as the home.works signed webhook). Set when
-- the deposit checkout opens; the webhook looks the quote up by it.
