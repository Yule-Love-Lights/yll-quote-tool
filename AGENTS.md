...
- **Never chain a state-changing command after one that can fail (a checkout, a cast, a cd): verify the intermediate state first, and memory-doc surgery goes through script files (python), never inline chains.** Reason: a chained write after a failed cast wiped task_ledger.md (S25), and a merge chained after a failed checkout landed on the wrong branch (unnumbered 2026-06-29 session).
