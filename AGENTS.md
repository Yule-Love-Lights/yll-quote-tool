...
- **A fix that adds a prop, param, or setting is inert until a caller passes it: grep the caller and prove behavior flips end-to-end — and for operator-facing changes the flip must be VISIBLE on the surface the operator uses.** A DB-correct change with no UI confirmation reads as broken and invites a harmful manual workaround. Reason: two "fixed" claims shipped doing nothing until review caught them (S29, S35), and the S49 lead→quote HL link wrote the DB correctly but showed nothing — the operator tested it as broken and re-picked by hand (the exact overwrite path the feature existed to remove).
