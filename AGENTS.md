...
- **Two devs + parallel sessions move the repo under you: re-scan `origin/master` + `gh pr list` immediately before building any ledgered task, and between waves of a long campaign.** Verify-before-build every ledgered task. Reason: duplicate PRs and one whole duplicated fix campaign shipped this way (S25 3x, S26, S35, S36).
- **Answer any current-state question from `git show origin/master:<path>` after a fetch, never a worktree copy.** Reason: stale answers cited deleted ledger rows and missed merged work (S0, S32, S38).
