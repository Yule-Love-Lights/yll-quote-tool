...
- **Lint:** `npm run lint` (`eslint-config-next`). The codebase uses targeted `// eslint-disable-next-line @next/next/no-img-element` only where a raw `<img>` is intentional (e.g. lazy video posters) — don't blanket-disable. **Note:** a fresh `npm install` pulls `eslint-plugin-react-hooks` v7 (via `eslint-config-next` 16), which enables the new `react-hooks/set-state-in-effect` rule at *error* level. Existing code predates it, so it's currently downgraded to **`warn`** in `eslint.config.mjs` to keep the gate green; a handful of effect sites still need refactoring before it can return to `error` (see `CURRENT_STATE.md`).
...
- **`FULL-SCHEMA.sql`** is a consolidated re-runnable snapshot, but it is **stale** as a full rebuild (dated Apr 22 — it omits `quotes`/base tables, the post-Apr-22 quotes columns, and `renders.variant`). A true from-scratch rebuild = `db/schema.sql` + `FULL-SCHEMA.sql` + the four later `quotes` migrations + `migrations/2026-05-29-reference-assets-create.sql`. **`reference_assets` is now captured in that migration** (its schema previously existed only in the live Supabase); refreshing the stale `FULL-SCHEMA.sql` itself is still worthwhile.
