...
- **TypeScript 5, `strict` mode.** The de-facto green-light gate before committing is **`npx tsc --noEmit`** (there is no `typecheck` npm script; Naldo runs `tsc` directly — it's been clean on every change). Keep it clean; don't merge type errors.
- **Lint:** `npm run lint` (`eslint-config-next`). The codebase uses targeted `// eslint-disable-next-line @next/next/no-img-element` only where a raw `<img>` is intentional (e.g. lazy video posters) — don't blanket-disable.
- **No pre-commit hooks are configured** (no husky/lint-staged in `package.json`). That means nothing stops a bad commit automatically — **run `npx tsc --noEmit` and `npm run lint` yourself before committing.** You should never need `--no-verify` (there are no hooks to skip); if you find yourself reaching for it, stop and ask why.
...
- **New portal section/skin:** base components in `src/components/portal/`; skin overrides in `portal/<skin>/`. Shared selection state is `SelectionContext.tsx`; portal-facing types are `components/portal/types.ts` (intentionally decoupled from `lib/pricing` types — the seam is `lib/portal/adapter.ts`). **`portal-snowglobe` (v6) is the designated live skin** (see `CURRENT_STATE.md` §4.1).
...
- **`FULL-SCHEMA.sql`** is a consolidated re-runnable snapshot, but it is **stale** as a full rebuild (dated Apr 22 — it omits `quotes`/base tables, the post-Apr-22 quotes columns, `renders.variant`, and `reference_assets`). A true from-scratch rebuild = `db/schema.sql` + `FULL-SCHEMA.sql` + the four later `quotes` migrations + a hand-made `reference_assets` table. **Capturing `reference_assets` in a migration is an open task** (see `CURRENT_STATE.md`).
