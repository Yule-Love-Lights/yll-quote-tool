...
- **Per-item rules** (mirrors `projectScene`'s per-unit branches + defaults so materials match pricing):
  - **Wreath** (`quoteSize` def `36noble`, `tier` def `bow`): base `wreath:<size>` ×1 + `wreath-bow:<size>` ×1 (a bow ships with EVERY wreath — both tiers, Naldo); if `fullDecor` also `wreath-fee:<size>` ×1. *(Garland differs — its bow gates on Decorated, per the "garland has no included bow" pricing rule; pending Naldo's confirm.)*
