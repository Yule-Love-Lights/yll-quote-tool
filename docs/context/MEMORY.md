...
> Claude Code auto-loads these at session start. **Read order:** this index → the newest `docs/context/journal/` fragments of BOTH devs (or the generated `JOURNAL.md`) → `project_quote_tool.md` (Current state / Decisions confirmed / Next up). See `README.md` for the continuity system + start/close protocols. `docs/context/` in the repo is the **canonical shared copy** (Jason + Naldo work on different machines; local memory is seeded from it and synced back to it).
>
> **🤝 Two devs (Jason + Naldo):** Naldo owns the dashboard (#58, `/`), Jason owns everything else. Full collaboration rules (branches, area ownership, review/merge, the anti-clobber memory convention) live in **`AGENTS.md` → "Multi-dev collaboration"**. **Per-session journal fragments** — each session writes ONLY its own `journal/S<N>-<dev>.md` (the old per-dev logs are frozen history, 2026-08-21); `task_ledger.md` + `project_quote_tool.md` stay unified (sync off fresh master so git auto-merges).

**Continuity (read first):**
- [Project state — READ FIRST](project_quote_tool.md) — current state at a glance, confirmed decisions (don't re-ask), run commands, gotchas, QA backlog, next up. *(Per-session "Done in SN" history split to [project_quote_tool_archive.md](project_quote_tool_archive.md), S16.)*
- [Journal fragments](journal/) — one file per session per dev (`S<N>-<dev>.md`); `JOURNAL.md` is the generated concatenation. Frozen history: [session_log.md](session_log.md), [session_log_naldo.md](session_log_naldo.md) + their archives.
- [Task ledger](task_ledger.md) — the single clean renumbered master task table; **ACTIVE = open/in-flight/backlog only** (lean). Completed + shelved rows → [task_ledger_archive.md](task_ledger_archive.md) (S16, read-on-demand). LIVING DOC — the source of truth for tasks.

> **🪶 S16 token-efficiency split (#94):** the ledger, Jason's session log, and project-state doc were trimmed to their ACTIVE content; full history lives in the three `*_archive.md` files above (read only when you need the back-story). See `AGENTS.md` → "Token-efficiency defaults".
...
- [Winback SMS campaign — STOPPED](project_winback_campaign.md) — Naldo's "database reactivation," run by Jason S31, stopped 2026-07-29 after tripping GHL's 6% opt-out compliance line (final: 5 conversions, ~$9.6k quoted, 1 booked). Read BEFORE any GHL bulk-SMS idea — the revival checklist + API gotchas live there. **Open money item: the texted "10% off" promise is missing from 4 of 5 campaign quotes (vahling booked at full price) — Jason/Naldo decision.**
