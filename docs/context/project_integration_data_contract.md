...
> **v0.5.1 NOTE (S6 Day 2, 2026-06-11 — quote-tool side only, nothing for the design tool to implement):** the `seed-*` convention now covers MORE than roofline strands. The **bridge auto-design** (#35 Phase 2) programmatically writes: per-unit items (`seed-mini-*` miniArea/strand, `seed-wreath-*`, `seed-spritzer-*`, `seed-garland-*` — replaced as a group on re-analysis; non-`seed-` items never touched) and a **scale yardstick `seed-yardstick-1`** (5 ft; width = derived px/ft × 5, from the AI's roofline-length ÷ footage calibration; only seeded when the scene has NO non-seed yardstick — staff calibration always wins). All are ordinary scene members per this contract; expect them in any quote-tool-created design.
>
> **v0.5 AMENDMENT (S6, 2026-06-10):** standalone bows price (#28) + roofline seeding (#33). NO scene-type changes — `BowItem` and the roofline `surface` tags already existed; this amendment changes projection semantics + adds a quote-side authoring convention:
...
- **Unmapped — RENDER but produce NO line item:**
  ...
  - *Standalone `bow`*: **prices as of v0.5 (#28, S6)** — one "Bow" line item per drawn bow at `standaloneBowPrice` (**$0 placeholder until Naldo prices it**). (Bows inside wreaths / on garland are priced via those items' `tier`/`withBow`, unchanged.)
