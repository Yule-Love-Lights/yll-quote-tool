...
> **✅ BUILT (Session 5, branch `jason/integration-projection`, PR'd):** the whole loop A1→D + A2 part 1. **B** `projectScene.ts` (scene→per-unit inputs + linkage). **C1** engine `customLineItems` + `/api/quote` server-side projection. **C3a** builder custom-items UI + `designId`. **A1** vendored the design-tool's gated Quote-binding `editor.ts @ 2233e1e` (the per-item surface/included + billed `quote*` panels). **D** portal toggle-filter (`sceneLinks.ts` + `hiddenSceneItemIds` + `render-readonly` hide). **A2 part 1** = v0.4 `MiniAreaItem`/`MiniGroupItem`/`MiniBilling`/`groupId` types + projection skip-logic. **Cemented decisions:** linkage = "live from design"; per-instance (1 drawn item = 1 line item = 1 toggle); **drawn size is VISUAL-ONLY → billed spec in staff-set `quote*` fields** (`quoteSize`/`quoteLength`/`quoteSections`); custom items have qty; no-design quotes keep manual entry (2a). Data contract → **v0.4**. Jason verified per-unit portal toggles live. **A2 part 2 (the area-tool editor.ts + `miniArea.ts` fill renderer) is being authored by the design-tool AI** → copy verbatim + wire into `render-readonly.ts`. **Deferred (after A2):** roofline portal PICTURE-toggle (needs c9 strands tagged santas/gingerbread).
