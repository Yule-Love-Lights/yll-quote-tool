...
- ~~**#17** Roofline redesign~~ **DONE (S2+S3)** — Phase 1+1b+2+4 all merged (PRs #12/#13/#14). AI red/blue + front-section-only validated S3 (classification confirmed; AI *accuracy* refinements → the AI-training-system task) · ~~**#18** $1000 minimum everywhere~~ **DONE (S2)** — reworked into a portal approval gate (not an engine floor) · **#19** [low] operator "recommend items" checkboxes
...
**Immediate next: AI training/correction SYSTEM task (Jason requested) — refine + improve.** The read-only walkthrough is DONE (S3); Jason now understands the loop and is compiling a list of questions/feedback/refinements (expect medium-large). **Workflow: collect his feedback FIRST, then design + build together.** The loop: `analyzePhoto` (`photoAnalysis.ts`) → corrections (`/quote/new` → `/api/save-correction` → `photo_corrections`) **or** training houses (`/training/new` → `/api/training` → `training_houses`) → few-shot replay (`correctionToExample`/`buildFewShotMessages`; **latest 2 corrections + 2 style-ranked training houses + 2 reference assets**, training first). Concrete inputs from Phase 4: corrections can't teach satellite (no satellite image stored — decide whether to store it / use `training_houses`); satellite front-edge/orientation logic needs work; street spatial grounding (lines hugging the roof) is the hard part. Tables: `photo_corrections`, `training_houses`, `reference_assets` (all RLS-disabled); ~42 old red=sides corrections exist but only the latest 2 are ever used.

After that: QA leftovers **#19** (operator recommend-items checkboxes), **#23** (re-analyze button), **#26** (portal color/pattern picker) + the Naldo-pending items.
