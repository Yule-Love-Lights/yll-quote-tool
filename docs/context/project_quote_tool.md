...
**Immediate next (Session 4): BUILD task #27 — design-tool integration, Phase 1.** Start from the locked data contract: create the Supabase `designs` table + scene storage, and port the design tool's Konva editor into the quote builder as a React component (manual, no AI). **Sequencing decision (S3):** going all-in on #27 is fine — it's PHASED so each phase ships value, and the roofline prerequisite (#7, the tier enum + footage split + front/sides tags) is already DONE. ⚠️ **PARK the portal-touching tasks until #27 reaches them** (esp. #10 portal color picker — the integration delivers color via the scene; and any "What's Included" toggle rework) to avoid throwaway work. Small measurement fixes (#25, #26) are SAFE anytime (optional palate-cleansers; #25 slightly helps #27's auto-scale). #8 feeds #27 Phase 3 (auto-design).
