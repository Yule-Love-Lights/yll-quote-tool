...
- **#17** Roofline redesign — **Phase 1+1b DONE (S2)** + **Phase 2 DONE (S3)** (portal switch) + **Phase 4 prompt/labels/renames DONE (S3, PR `jason/roofline-phase4`)** (AI red/blue + front-section-only + downspout exclusion; ⚠️ AI output validation pending the training-data wipe) — roofline epic complete bar that validation · ~~**#18** $1000 minimum everywhere~~ **DONE (S2)** — reworked into a portal approval gate (not an engine floor) · **#19** [low] operator "recommend items" checkboxes
...
**Immediate next: the training/correction SYSTEM task (Jason requested) — explain THEN wipe + refill.** Phase 4's prompt is correct but its AI output is corrupted by old few-shot data (all old corrections use red=sides). Before Jason wipes the data, walk him through the whole loop end-to-end: `analyzePhoto` (`photoAnalysis.ts`) → operator corrections (`/quote/new`, `/training/new`) → `corrections.ts`/`training.ts` storage (which Supabase tables/rows) → few-shot feedback (`correctionToExample`/`buildFewShotMessages`) → next analysis. Then he wipes + refills with correct red/blue corrections, and we **validate Phase 4's tracing on clean data** (tune the prompt if needed). This is the real unblock for Phase 4.
