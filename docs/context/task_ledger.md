...
|---|------|------|--------|-------|
| 8 | AI training/correction system refinement → detail in [[task_ai_training_refinement]] | L | **Stage A MERGED (S7, PR #30)** — scene-based capture + few-shot. **Stage B SHIPPED (S8)** — Voyage+pgvector similarity retrieval, unified pipeline; re-verified live + committed on `jason/few-shot-retrieval` (`e9a09e7`/`cdfe543`/`2f2cd3e`+docs), PR off master (merge pending). **Stage C (designer-brain prompt) = next.** | #28 |
