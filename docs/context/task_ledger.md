...
| 36 | **Gemini/AI render pipeline TEARDOWN** (raised S6, Jason) — remove the whole stack. **PLAN APPROVED-READY (S6 Day 2; recon DONE — full inventory in the workflow journal `…\subagents\workflows\wf_47595b65-18f\journal.jsonl`, 3 structured reports):** DELETE `src/lib/rendering/*` (8 files), 4 render API routes (`/api/renders`, `/api/renders/[id]`, `/api/quotes/[id]/renders`, `/api/quotes/[id]/render-variants`), 3 admin pages (`admin/renders`, `admin/renders/new`, `admin/quotes/[id]/renders`) — zero outside importers. MODIFY: `portal/photos.ts` (drop the renders query → null URLs; hero falls back design→static, already safe), DELETE `PackageVariantGallery` + `variantPhoto.ts` + the `variantPhotos` plumbing (types/adapter/portal page mount — go beyond the scouts' "keep-inert"), remove admin "Renders" nav links. INFRA: write `migrations/2026-06-11-drop-renders.sql` (drop table + 3 RLS policies + bucket policies; update FULL-SCHEMA) — Jason applies + deletes the `renders` bucket in the UI; THEN remove env vars `GEMINI_API_KEY`, `RENDER_MODEL`, `RENDER_BUDGET_MONTHLY_USD`, `REPLICATE_API_TOKEN` + 3 render-tuning vars (exact names in the journal) from `.env.local` + Vercel. **KEEP:** `GOOGLE_MAPS_API_KEY`, `ANTHROPIC_API_KEY`, `sharp`, `@anthropic-ai/sdk` (no Gemini/Replicate npm packages exist — raw fetch). **VIDEO feature stays** (recon: zero render deps — re-verify with one grep at build). Historical render migrations stay in git. Customer effect: design quotes untouched; no-design quotes → static fallback hero, no gallery. | M–L | **EXECUTE NEXT SESSION (S7)** — plan locked, recon journal on disk | — |
