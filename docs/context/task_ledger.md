...
| 36 | **Gemini/AI render pipeline TEARDOWN** — executed from the S6-locked plan: DELETED `src/lib/rendering/*` (8 files), the 4 render API routes, the 3 admin render pages, `PackageVariantGallery` + `variantPhoto.ts` + all `variantPhotos` plumbing (both components were already orphaned); `portal/photos.ts` → pure null-URL fallback (hero: live design → static); admin "Renders" nav link removed; `migrations/2026-06-12-drop-renders.sql` (Jason applies + deletes the `renders` bucket in the Supabase UI); FULL-SCHEMA purged; `.env.local.example` + docs scrubbed. 7 env vars removed (`GEMINI_API_KEY`, `RENDER_MODEL`, `RENDER_VARIANT_MODEL`, `RENDER_VARIANT_CACHE_BUST`, `RENDER_BUDGET_MONTHLY_USD`, `REPLICATE_API_TOKEN`, `REPLICATE_INPAINT_MODEL`); KEPT `GOOGLE_MAPS_API_KEY`/`ANTHROPIC_API_KEY`/sharp/`@anthropic-ai/sdk`. Video feature untouched (grep-verified). Closed #18 as moot. | M–L | S7 | — |
