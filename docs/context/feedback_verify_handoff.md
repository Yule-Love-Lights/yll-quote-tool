...
**FORMAT them as real clickable Markdown links** (2026-06-16, explicit re-ask — bare/inline-code URLs weren't clickable for him). Write `[http://localhost:3000/quote/new](http://localhost:3000/quote/new)` (or a labelled `[Satellite tab](http://localhost:3000/quote/new)`) — NOT a bare URL and NOT inside backticks (`` `...` `` renders as non-clickable code). This applies to verification links AND any PR/compare links. The terminal renders Markdown, so link them so he can click straight through.
