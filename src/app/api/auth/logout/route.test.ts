...
// unconfigured environment). Supabase mocked. Also (WT-62): must clear the
// staff-device marker cookie so it doesn't outlive the session.
...
describe('POST /api/auth/logout', () => {
  ...
  it('clears the staff-device cookie (WT-62 — it otherwise outlives the session by up to a year)', async () => {
    const res = await POST();
