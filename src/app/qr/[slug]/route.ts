...
//
// The codes on the truck decals, business cards and flyers all point at
// https://link.yulelovelights.com/qr/<slug>, a subdomain that used to be served
// by a GoHighLevel account a former agency opened and that is now attached to
// no account at all. Pointing that subdomain at this app is what brings the
// printed material back to life; this route is what answers it.
//
// All of the reasoning (why it never 404s, why the redirect is 302 and not 301)
// lives with the logic in src/lib/qrRedirect.ts, which is where the tests are.
// Allowlisted for signed-out access in src/lib/auth/operatorGate.ts in this same
// change - without that the gate serves the operator login shell to a homeowner
// holding a business card, which looks exactly like the outage we are fixing.
