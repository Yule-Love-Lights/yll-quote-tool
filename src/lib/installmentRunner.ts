...
//
// THIS MOVES REAL MONEY. Three separate gates stand between this code and a
// customer's card, and ALL THREE must be open:
...
// And a fourth, outside this file: NOTHING IS SCHEDULED. `vercel.json` has no
...
import {
  chargeBalanceOnFile,
  describeChargeSlot,
  CHARGE_SLOT_STALE_MS,
} from '@/lib/integrations/valorBalance';
...
export function planInstallmentRun(plans: InstallmentPlan[], asOf: Date): RunDecision[] {
  return plans.map((plan) => {
    ...
    } else if (slot.kind === 'in-flight' && !slot.stale) {
      reasons.push('already-charging');
      details.push(
        `a charge claimed at ${slot.sinceIso} is still in flight (claims go stale after ${Math.round(CHARGE_SLOT_STALE_MS / 60000)} minutes)`,
      );
    }
...
async function chargeOne(sb: ServiceClient, d: RunDecision): Promise<ChargeOutcome> {
  ...
  if (!result.ok) {
    ...
    if (ambiguousTimeout) {
      // Money may have moved. Leave the claim so nothing retries inside the
      // stale window, and say so loudly.
      return {
        ...base,
        status: 'charged-not-recorded',
        txnId: null,
        message: `Valor timed out — the charge may have landed. Check Valor before doing anything else; the claim is deliberately left in place. (${result.message})`,
      };
    }
  ...
  // Partial auth: a card-on-file sale can capture LESS than requested. Recording
  // the full amount then would overstate what we collected. Mirrors the
  // charge-balance route's guard.
  if (result.chargedUsd == null || result.chargedUsd + 0.005 < amountUsd) {
    return {
      ...base,
      status: 'charged-not-recorded',
      txnId: result.txnId,
  ...
  if (!recorded.ok) {
    return {
      ...
      status: 'charged-not-recorded',
      txnId: result.txnId,
      message: `The card WAS charged (txn ${result.txnId ?? 'unknown'}) but recording it failed: ${recorded.error}`,
    };
  }
 ...
 *
 * A run that charged nothing and hit no problem sends NOTHING — the same
 * quiet-day rule the completing-today ping uses. A DECIDED-nothing day is the
 * norm for eleven months of the year.
 */
export function runSummaryMessage(result: RunResult): string | null {
  ...
  if (!charged.length && !problems.length) return null;
