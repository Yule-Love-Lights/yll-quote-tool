...
// Valor signs each webhook with HMAC-SHA256 using a shared secret. Headers:
//   Valor-Signature  — hex (or base64) HMAC digest
//   Valor-Timestamp  — unix seconds; bound the replay window
//
// We MUST verify before acting — an unverified webhook handler is an
// "anyone can mark any quote paid" hole. We compute the digest over a couple of
// the common signing-base conventions and accept if any matches, in constant
// time, so the exact convention (which we couldn't confirm from a live probe)
// doesn't block launch. CONFIRM the exact base + encoding at live test, then
// drop the extras.
...
export function verifyWebhookSignature(input: WebhookVerifyInput): boolean {
  ...
  // Replay protection — reject stale timestamps when one is provided.
  if (timestamp) {
  ...
  // Candidate signing bases (most specific first). `${ts}.${body}` is the
  // Stripe-style scheme; raw-body-only is the other common one.
