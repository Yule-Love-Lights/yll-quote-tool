...
export async function createHostedPageSale(input: HostedPageInput): Promise<HostedPageResult> {
  ...
  const body: Record<string, unknown> = {
    ...
    // every human leg with either a 302 to their real portal confirmation
    // (when the order ref resolves a quote id — the `bal_` balance-pay-link
    // convention) or a 200 minimal branded "payment received" page (#171e,
    // the unrecoverable/error case) — never a raw JSON/error response, and
    // never `/` (the operator login). Only when VALOR_REDIRECT_CAPTURE_URL is
    // set does redirect_url point at our capture route
    // (src/app/api/integrations/valor/redirect-capture/route.ts) instead of
    // success_url. See #161 in the ledger.
    shipping_country: 'US',
