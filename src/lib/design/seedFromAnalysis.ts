 ...
 * auto-yardstick (same 5 ft realFeet, same `seed-` id, same lower-left corner
 * position) MINUS the AI calibration: the width is a fixed FRACTION of the photo
 * (a visible, proportional handle), so the operator sizes it against a known
 * object (garage door ≈ 16 ft, etc.). The `seed-` id keeps "staff calibration
 * wins" intact — a hand-dropped yardstick (non-seed id) still supersedes it.
 */
