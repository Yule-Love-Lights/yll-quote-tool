...
import { priceSelection, chargesFromResult, minimumOrderSubtotal } from './derivePackages';
import { BUSINESS_RULES, type QuoteResult } from '@/lib/pricing/pricingEngine';
import type { PortalCharges, PortalLineItem } from '@/components/portal/types';

// Standard tax, no per-job fees — the common case.
const PLAIN: PortalCharges = { rushFee: 0, takedown: 0, taxRate: 0.08625 };
...
describe('priceSelection — real price, no $1,000 floor (#18)', () => {
  ...
  it('adds rush + takedown to the taxable amount, then taxes', () => {
    const charges: PortalCharges = { rushFee: 150, takedown: 150, taxRate: 0.08625 };
  ...
  it('Subtotal + fees + Tax tie out to the Total (so the breakdown is honest)', () => {
    const charges: PortalCharges = { rushFee: 150, takedown: 0, taxRate: 0.08625 };
...
describe('chargesFromResult', () => {
  it('maps rush/takedown and derives the effective tax rate from the quote', () => {
    const charges = chargesFromResult(
      resultWith({ rushFeeAmount: 150, takedownAmount: 0, taxableAmount: 1000, taxAmount: 86.25 }),
    );
    expect(charges.rushFee).toBe(150);
    expect(charges.takedown).toBe(0);
