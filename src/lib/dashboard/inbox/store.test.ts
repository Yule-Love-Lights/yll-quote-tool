...
describe('ensureFollowUp — idempotency scoped to pending (WT-43)', () => {
  ...
  function makeFollowUpsFake(seed: FollowUpRow[]) {
    ...
    function table() {
      ...
      // Tracks a `.select(...)` chained AFTER `.update(...)` (markFollowUpDone's
      // `.update({status:'done'}).eq('id',id).select('inbox_item_id')` shape) —
      // real Supabase returns the updated row(s) back in that case instead of
      // `data: null`. Must NOT clobber `mode`, or the .then() handler below
      // would take the generic select branch and never apply the update.
      let selectRequested = false;
      const self: Record<string, unknown> = {};
      self.select = () => {
        selectRequested = true;
        if (mode !== 'update' && mode !== 'upsert') mode = 'select';
        return self;
      };
      ...
      // Mirrors real supabase-js: resolves the single matching row (or null),
      // not an array. Terminal call — built on top of .then() below.
      self.maybeSingle = () =>
        new Promise((resolve) => {
          (self.then as (r: (v: unknown) => void) => void)((result) => {
            const { data, error } = result as { data: unknown; error: unknown };
            const single = Array.isArray(data) ? (data[0] ?? null) : data;
            resolve({ data: single, error });
          });
        });
      self.then = (resolve: (v: unknown) => void) => {
        if (mode === 'insert') {
          ...
          resolve({ data: selectRequested ? matched : null, error: null });
        } else if (mode === 'upsert') {
  ...
  // #252 slice E: markFollowUpDone now hands back the follow-up's anchored
  // inbox_item_id so a caller (the /api/dashboard/followup route) can decide
  // whether to also resolve that item — see the route.test.ts coupling tests.
  ...
  it('accepts a null operatorId (auth gate dormant) and logs the activity as system', async () => {
    const fake = makeFollowUpsFake([
      { id: 'fu-1', inbox_item_id: 'item-1', reason: 'quote_sent_no_reply', status: 'pending' },
    ]);
