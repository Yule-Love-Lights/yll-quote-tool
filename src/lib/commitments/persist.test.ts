...
// call_commitments_finalize_extraction with the right args and interprets
// every documented result ('ok' / 'already_finalized' / 'refused' /
// anything else) correctly. The RPC's own atomicity/TOCTOU-closing logic is
// NOT retested here (it cannot run under vitest -- see this slice's build
// notes for the line-by-line cross-read against the copilot source that
// verifies it instead).
...
describe('persistCommitments', () => {
  it('calls the finalize RPC with the transcript id, mapped rows, and extractor version', async () => {
    const { client, rpc } = fakeSupabase({ data: 'ok', error: null });

    await persistCommitments(client, 't-1', 'rep@example.com', 'contact-1', ROWS, 'claude-haiku-4-5-20251001');
  ...
  it('returns ok/alreadyFinalized=false on "ok"', async () => {
    ...
    const result = await persistCommitments(client, 't-1', null, null, ROWS, 'v1');
  ...
  it('throws on a Postgres/PostgREST error', async () => {
    ...
    await expect(persistCommitments(client, 't-1', null, null, ROWS, 'v1')).rejects.toEqual({ message: 'boom' });
  });
  ...
  it('throws on an unexpected (unrecognized) RPC result rather than silently succeeding', async () => {
    ...
    await expect(persistCommitments(client, 't-1', null, null, ROWS, 'v1')).rejects.toThrow(
